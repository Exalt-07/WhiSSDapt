...
# -------- TRAIN DATA --------
    if args.is_train:
        ...
        if args.dataset_train in ["ITA-stress", "GER-stress"]:
            train = DatasetTrain.dataset["train"]
            val = None
        else:
            train, val = DatasetTrain.split_train_val()
...
# -------- EVAL / TEST DATA --------
    ...
    if args.dataset_eval in ["ITA-stress", "GER-stress"]:
        test = DatasetEval.dataset["test"]
    else:
    ...
    training_args = Seq2SeqTrainingArguments(
        ...
        warmup_ratio=0.05,
        num_train_epochs=6, # Increase to 4 for task-specific evaluation, use 2 for zero-shot - More generalized evaluation. 
        seed=42,
    ...
    if args.is_train:
        ...
        train_dataset=train,
        eval_dataset=val if args.dataset_train != "ITA-stress" else None,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset_train",
        type=str,
        choices=["tinyStress-15K","ITA-stress","GER-stress"], # add other datasets as needed
